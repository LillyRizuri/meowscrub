...
module.exports = class RoleInfoCommand extends Commando.Command {
    constructor(client) {
        super(client, {
            ...
            examples: ['roleinfo Member', 'roleinfo @Member', 'roleinfo 694239225226199070'],
            guildOnly: true
        })
    }
    ...
    run(message, args) {
        ...
        if (message.mentions.roles.first()) {
            ...
            role = message.guild.roles.cache.find(e => e.name.toLowerCase().trim() == args.toLowerCase().trim())
            if (!role) return message.reply(invalidEmbed)
        } else if (args && !isNaN(args)) {
            role = message.guild.roles.cache.find(e => e.id == args)
            if (!message.guild.roles.cache.has(args)) return message.reply(invalidEmbed)
        }

        if (!role) {
            const noValueEmbed = new Discord.MessageEmbed()
                .setColor(what)
                .setDescription("<:scrubnull:797476323533783050> Insert a role name or equivalent to continue.")
...
• Hoisted: \`${role.hoist.toString().replace("true", "Yes").replace("false", "No")}\`  
• Position: \`${role.position}\`  
        ...
        switch (role.members.size) {
            case 0:
                ...
                break
            case 1:
                roleInfoEmbed.setDescription(`**1** person has the <@&${role.id}> role.`)
                break
            default:
                roleInfoEmbed.setDescription(`**${role.members.size}** people have the <@&${role.id}> role.`)
                break
        }

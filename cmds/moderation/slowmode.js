...
const { what, green, red } = require('../../colors.json')
    ...
    async run(message, args) {
        ...
        if (!args[0]) {
            const noTimeSetEmbed = new Discord.MessageEmbed()
                .setColor(what)
                .setDescription("<:scrubnull:797476323533783050> What do you want the slowmode to be set to?")
        ...
        if (isNaN(cooldownValue)) {
            const notValidTimeEmbed = new Discord.MessageEmbed()
                .setColor(red)
                .setDescription("<:scrubred:797476323169533963> Breaking me using value like that? No.")
                .setFooter('heh.')
                .setTimestamp()
        ...
        if (!Number.isInteger(cooldownValue)) {
            const notIntegerEmbed = new Discord.MessageEmbed()
                .setColor(red)
                .setDescription("<:scrubred:797476323169533963> Cooldown can't be applied if the value isn't an integer.")
                .setFooter("ya bafoon")
                .setTimestamp()
        ...
        if (args[0] < 0 || args[0] > 21600) {
            const notInBetweenEmbed = new Discord.MessageEmbed()
                .setColor(red)
                .setDescription("<:scrubred:797476323169533963> The cooldown value should be in-between 0 and 21600.")
                .setFooter('try again.')
                .setTimestamp()
        ...
        const slowModeOKEmbed = new Discord.MessageEmbed()
            .setColor(green)
            .setDescription(`<:scrubgreen:797476323316465676> Slowmode for ${channel} has been set to **${args[0]}** second(s).`)

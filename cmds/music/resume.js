...
const { embedcolor, red, what, green } = require('../../assets/json/colors.json')
    ...
    run(message) {
        ...
        if (playing === true) {
            const alreadyPausedEmbed = new Discord.MessageEmbed()
                .setColor(red)
                .setDescription(`<:scrubred:797476323169533963> What's the point if the speaker is already playing.`)
                .setFooter("jeez")
                .setTimestamp()
        ...
        switch (paused) {
            case true:
                ...
                const stoppedEmbed = new Discord.MessageEmbed()
                    .setColor(green)
                    .setDescription('<:scrubgreen:797476323316465676> **Resumed the track.**')

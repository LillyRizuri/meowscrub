...
const { what, red, green } = require('../../colors.json')
    ...
    async run(message, args) {
        ...
        if (!queue) {
            const noMusicEmbed = new Discord.MessageEmbed()
                .setColor(what)
                .setDescription("<:scrubnull:797476323533783050> There's no queue. Have at least one song before advancing.")
                .setFooter("c'mon")
                .setTimestamp()
        ...
        if (isNaN(volume) || !Number.isInteger(volume)) {
            const invalidEmbed = new Discord.MessageEmbed()
                .setColor(red)
                .setDescription(`<:scrubred:797476323169533963> Percentage not valid. Try again.`)
                .setFooter("bus")
                .setTimestamp()
        ...
        if (volume < 1 || volume > 200) {
            const invalid2Embed = new Discord.MessageEmbed()
                .setColor(red)
                .setDescription(`<:scrubred:797476323169533963> The percentage you provided must be in-between 1% and 200%`)
        ...
        const volEmbed = new Discord.MessageEmbed()
            .setColor(green)
            .setDescription(`<:scrubgreen:797476323316465676> Adjusted the volume to **${volume}%**.`)

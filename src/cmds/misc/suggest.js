  ...
  async run(message, args) {
    ...
    if (!channel) {
      const noSuggestChannelEmbed = new Discord.MessageEmbed()
        .setColor(what)
        .setDescription(
          "<:scrubnull:797476323533783050> This server doesn't have any suggestion channel set up."
        )
    ...
    channel.send(embed).then((msg) => {
      const successfulEmbed = new Discord.MessageEmbed()
        .setColor(green)
        .setDescription(
          `<:scrubgreen:797476323316465676> Successfully recorded your suggestion into ${channel}.`
        )
        .setFooter("nice!")
        .setTimestamp();

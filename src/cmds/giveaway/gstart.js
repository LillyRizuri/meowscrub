  ...
  async run(message, args) {
    ...
    if (args.length < 4) {
      const notEnoughEmbed = new Discord.MessageEmbed()
        ...
        .setFooter(
          `Example: ${message.guild.commandPrefix}gstart #giveaway 30m 5 Awesome T-Shirt`
        )
        .setTimestamp();
    ...
    this.client.giveawaysManager
      ...
      .then(() => {
        const confirmEmbed = new Discord.MessageEmbed()
          .setColor(green)
          .setDescription(
            `<:scrubgreen:797476323316465676> Giveaway established in ${channel} for \`${giveawayPrize}\`.`
          )
      ...
      .catch((err) => {
        const errorEmbed = new Discord.MessageEmbed()
          .setColor(red)
          .setDescription(
            `<:scrubred:797476323169533963> Illegal arguments detected. Please try again.`
          )
          .setFooter(err)
          .setTimestamp();

...
module.exports = (client, message) => {
  ...
  const welcomeMsgEmbed = new Discord.MessageEmbed()
    .setColor(embedcolor)
    .setAuthor(`A message from ${author}`)
    .setTitle(
      `thank you for adding ${client.user.username} to this server.`
    )
    .setThumbnail(client.user.displayAvatarURL())
    .addFields(
      {
        name: "Getting Started",
        value: `Use \`${message.guild.commandPrefix}help\` to let the bot provide the full list of commands available. Hope you utilize all commands present in there.`,
      },
      {
